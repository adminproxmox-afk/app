(function () {
  ...
  const text = {
    uk: {
      ...
      defaultHint: 'Оберіть елемент, щоб застосувати його. Нові речі відкриваються у магазині.',
      ownedItems: 'елементів у гардеробі',
      active: 'Активно',
      available: 'Доступно',
      shopOnly: 'У магазині',
      shopHint: 'Нові речі купуються у магазині.',
      selected: 'Вибрано',
      ownedAria: 'власний',
      coins: 'монет',
    ...
    en: {
      ...
      defaultHint: 'Choose an item to apply it. New items are unlocked in the shop.',
      ownedItems: 'items in wardrobe',
      active: 'Active',
      available: 'Available',
      shopOnly: 'In shop',
      shopHint: 'New items are purchased in the shop.',
      selected: 'Selected',
      ownedAria: 'owned',
      coins: 'coins',
  ...
  function loadState() {
    ...
    if (character && typeof character === 'object') {
      ...
      base.owned = Array.isArray(character.owned) ? character.owned : base.owned;
    }

    if (saved) {
      base.activeCategory = saved.activeCategory || base.activeCategory;
      base.selected = { ...base.selected, ...(saved.selected || {}) };
      base.owned = Array.isArray(saved.owned) ? saved.owned : base.owned;
    }
  ...
  function renderItems() {
    ...
    const categoryItems = items[state.activeCategory] || [];

    elements.categoryTitle.textContent = category ? trChar(category.label) : text.defaultTitle;
    elements.categoryHint.textContent = category ? `${trChar(category.hint)} ${text.shopHint}` : text.defaultHint;
    ...
    categoryItems.forEach((item) => {
      ...
      card.type = 'button';
      card.className = `item-card${active ? ' is-active' : ''}${owned ? ' is-owned' : ''}`;
      card.setAttribute('aria-label', `${trItem(item.label)} ${owned ? text.ownedAria : `${item.price} ${text.coins}`}`);
      ...
      price.textContent = owned ? (active ? text.active : text.available) : text.shopOnly;
